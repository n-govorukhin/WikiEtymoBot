...
По умолчанию режим размышлений включается только на этапе генерации ответа, но этим можно управлять с помощью параметра
`--reasoning`.
Например, `--reasoning reference_inserter` включает размышления перед вставкой сносок на источники, а `--reasoning off`
выключает их везде.
...
`~/.wikietymobot/cache/`.
Отключить кеширование можно с помощью опций `--no-cache http` и `--no-cache llm`.


## 🚧 Ограничения

- LLM может делать ошибки в ответе, поэтому для полной уверенности рекомендуется проверять источники вручную.
  Вероятность генерации полностью ложных или необоснованных ответов сведена к минимуму, однако в редких случаях ответ
  может содержать мелкие ошибки, например, перепутанные местами слова, неправильно переведённые термины и тому подобное.
